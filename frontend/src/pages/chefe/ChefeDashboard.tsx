...
export default function ChefeDashboard() {
  ...
  const [currentView, setCurrentView] = useState<
    "dashboard" | "users" | "instituicoes" | "estagios"
  >("dashboard");
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      setError((e) => ({
        ...e,
        users: err?.message || "Erro ao carregar utilizadores.",
      }));
    } finally {
  ...
  const fetchInstituicoes = useCallback(async () => {
    ...
    try {
      ...
      setError((e) => ({
        ...e,
        instituicoes: err?.message || "Erro ao carregar instituições.",
      }));
    } finally {
  ...
  const fetchEstagios = useCallback(async () => {
    ...
    try {
      ...
      setError((e) => ({
        ...e,
        estagios: err?.message || "Erro ao carregar estágios.",
      }));
    } finally {
  ...
  const fetchDepartamentos = useCallback(async () => {
    ...
    try {
      ...
      setError((e) => ({
        ...e,
        departamentos: err?.message || "Erro ao carregar departamentos.",
      }));
    } finally {
  ...
  const openEditInst = (inst: Instituicao) => {
    setInstForm({
      ...
      phone: inst.phone || "",
      email: inst.email || "",
      ponto_focal_nome: inst.tutors?.[0]?.user?.name || "",
      ponto_focal_contacto: inst.tutors?.[0]?.user?.email || "",
    });
        ...
        <div className="content">
              ...
              <div className="section-header">
                <div>
                  ...
                  <div className="page-subtitle">
                    Bem-vindo ao Sistema de Estágios Profissionalizantes
                  </div>
                </div>
              </div>
          ...
          {currentView === "users" && (
            <div>
              ...
              {error.users && (
                <div className="alert alert-error">{error.users}</div>
              )}
              <div className="card">
                      ...
                      {users.length === 0 ? (
                        ...
                        users.map((u) => (
                          <tr key={u.id}>
                            ...
                            <td>
                              <span
                                className={`badge ${u.status === "active" ? "badge-green" : "badge-muted"}`}
                              >
                                {u.status || "active"}
                              </span>
                            </td>
            ...
            <div>
              <div className="section-header">
                <div>
                  ...
                  <div className="page-subtitle">
                    Registar, editar e remover instituições
                  </div>
                </div>
              ...
              {error.instituicoes && (
                <div className="alert alert-error">{error.instituicoes}</div>
              )}
              <div className="card">
                <div className="table-wrap">
                  <table>
                      ...
                      {filteredInst.length === 0 ? (
                        ...
                        filteredInst.map((inst) => (
                          <tr key={inst.id}>
                            ...
                            <td>{inst.phone || "—"}</td>
                            <td>
                              {(inst.tutors?.[0]?.user?.name ?? "—") || "—"}
                            </td>
                            <td>
                              {(inst.tutors?.[0]?.user?.email ?? "—") || "—"}
                            </td>
          ...
          {currentView === "estagios" && (
            <div>
              ...
              {error.estagios && (
                <div className="alert alert-error">{error.estagios}</div>
              )}
              <div className="card">
                <div className="table-wrap">
                  <table>
                    ...
                    <tbody>
                      {estagios.length === 0 ? (
                        ...
                        estagios.map((e) => (
                          <tr key={e.id}>
                            <td>{e.tutor?.institution?.name || "—"}</td>
                            <td>{e.supervisor.user.name || "—"}</td>
                            <td>
                              {e.student.user.name || e.user?.name || "—"}
                            </td>
                            <td>
                              <span
                                className={`badge ${e.status === "ATIVO" ? "badge-green" : "badge-yellow"}`}
                              >
                                {e.status || "PENDENTE"}
                              </span>
                            </td>
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    value={instForm.nuit}
                    onChange={(e) =>
                      setInstForm((f) => ({ ...f, nuit: e.target.value }))
                    }
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setInstForm((f) => ({
                        ...f,
                        ponto_focal_nome: e.target.value,
                      }))
                    }
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setInstForm((f) => ({
                        ...f,
                        ponto_focal_contacto: e.target.value,
                      }))
                    }
